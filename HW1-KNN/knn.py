...
class KNN:
    ...
    def predict(self, features):
        ...
        for i in range(length):
            temp_label = self.get_k_neighbors(features[i])
            result.append(Counter(temp_label).most_common(1)[0][0])
        return result
    ...
    def get_k_neighbors(self, point):
        ...
        :return:  List[int]
        """
        length = len(self.train_features)
        neighbor_distance = []

        for i in range(length):
            distance = self.distance_function(point, self.train_features[i])
            neighbor_distance.append([self.train_labels[i], distance])
        neighbor_distance.sort(key=lambda x: x[1])
        neighbor_label = []
        for j in range(self.k):
            neighbor_label.append(neighbor_distance[j][0])
